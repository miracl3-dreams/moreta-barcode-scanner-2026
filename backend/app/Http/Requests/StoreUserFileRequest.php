<?php

namespace App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class StoreUserFileRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user() !== null;
    }

    /**
     * @return array<string, mixed>
     */
    public function rules(): array
    {
        return [
            'usrcde' => ['required', 'string', 'max:15', Rule::unique('userfile', 'usrcde')],
            'usrname' => ['required', 'string', 'max:30'],
            'usrpwd' => ['required', 'string', 'max:30'],
            'usrlvl' => ['nullable', 'string', 'max:15'],
            'brnchcde' => ['nullable', 'string', 'max:50'],
        ];
    }

    /**
     * @return array<string, string>
     */
    public function messages(): array
    {
        return [
            'usrcde.unique' => 'Code already exists in database!',
            'usrcde.required' => 'This field cannot be blank',
            'usrname.required' => 'This field cannot be blank',
            'usrpwd.required' => 'This field cannot be blank',
        ];
    }
}
