...
class CustomerActivityLogRepository
{
    /**
    ...
    /**
     * @return Collection<int, CustomerActivityLog>
     */
    ...
    private function filteredQuery(string $search, string $sort = '', string $dir = 'asc')
    {
        [$column, $direction] = CrudList::order(
            $sort,
            $dir,
            ['usrdte', 'usrtim', 'cuscde', 'activity', 'remarks'],
            'usrdte',
        );
        $query = CustomerActivityLog::query()->api()->orderBy($column, $direction);
